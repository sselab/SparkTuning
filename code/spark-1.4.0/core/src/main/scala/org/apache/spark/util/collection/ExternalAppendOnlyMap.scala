...
class ExternalAppendOnlyMap[K, V, C](
  ...
  private var hasHash:Boolean=false
  /**
  ...
  def insertAll(entries: Iterator[Product2[K, V]]): Unit = {
    ...
    //trace
      if(InternalProfiler.getTaskProfileForThreadId(Thread.currentThread().getId).hashToAggreatorMap.containsKey(this.hashCode())){
          hasHash=true
      }
      var shuffle:Object=null
      if(hasHash) {
        shuffle = InternalProfiler.getTaskProfileForThreadId(Thread.currentThread().getId).getShuffleReadOrWriteFromMap(this.hashCode())
    ...
    while (entries.hasNext) {
     ...
     // computeTime+=System.nanoTime()-startTime
      ...
      cCombPhaseTime+=System.nanoTime()-startTime1
      addElementsRead()
    }
    computeTime=System.nanoTime()-computeStartTime-cCombPhaseTime-cSpillPhaseTime
    ...
    dSpillRecs+=currentMap.size
    if(dSpillRecs!=0) {
      objectSize = (currentMap.estimateSize() + memoryBytesSpilled) / dSpillRecs
    }
    if(hasHash) {
      if (shuffleRead != null) {
        ...
        shuffleRead.cCombPhaseTime = cCombPhaseTime
        shuffleRead.dPerSpillRecs = dPerSpillRecs
        shuffleRead.cSpillPhaseTime = cSpillPhaseTime - cSortPhaseTime
        ...
        shuffleWrite.cSpillPhaseTime = cSpillPhaseTime - cSortPhaseTime
  ...
  override def iterator: Iterator[(K, C)] = {
    //trace
    val iter =if (spilledMaps.isEmpty) {
      if(shuffleRead!=null){
        shuffleRead.dCombRecs=currentMap.size
      }
      currentMap.iterator
    } else {
  ...
  private class ExternalIterator extends Iterator[(K, C)] {
    ...
    private val sortedMap = {
      val startTime=System.nanoTime()
      val iter=currentMap.destructiveSortedIterator(keyComparator)
      cSortPhaseTime+=System.nanoTime()-startTime
      iter
    }
    ...
    private def readNextHashCode(it: BufferedIterator[(K, C)], buf: ArrayBuffer[(K, C)]): Unit = {
      ...
      if(hasHash) {
        if (shuffleWrite != null) {
          shuffleWrite.cMergeReadTime += System.nanoTime() - startTime
        } else {
          shuffleRead.cMergeReadTime += System.nanoTime() - startTime
        }
      }
    }
    ...
    private def mergeIfKeyExists(key: K, baseCombiner: C, buffer: StreamBuffer): C = {
      ...
      if(hasHash) {
        if (shuffleWrite != null) {
          ...
          // cMergeCombTime+=time
          shuffleWrite.cMergeCombTime += time
        } else {
          val time = System.nanoTime() - startTime
          // cMergeCombTime+=time
          shuffleRead.cMergeCombTime += time
        }
      }
      baseCombiner
    }
    ...
    override def hasNext: Boolean = {
      ...
      if(mergeHeap.length>0){
        ...
        if(hasHash) {
          if (shuffleWrite != null) {
            shuffleWrite.cMergeSortTime -= (shuffleWrite.cMergeReadTime + shuffleWrite.cMergeCombTime)
          } else {
            System.out.println("sort      "+shuffleRead.cMergeSortTime);
            System.out.println("read      "+shuffleRead.cMergeReadTime);
            System.out.println("combine   "+shuffleRead.cMergeCombTime);
            shuffleRead.cMergeSortTime -= (shuffleRead.cMergeReadTime + shuffleRead.cMergeCombTime)
          }
        }
        cMergeReadTime=0
        cMergeCombTime=0
    ...
    override def next(): (K, C) = {
      ...
      if(hasHash) {
        if (shuffleWrite != null) {
          shuffleWrite.cMergeSortTime += System.nanoTime() - startTime
        } else {
          shuffleRead.cMergeSortTime += System.nanoTime() - startTime
          shuffleRead.dCombRecs += 1
        }
      }
      (minKey, minCombiner)
    }
