...
import org.apache.spark.{Logging, TaskContextImpl, TaskContext}
...
private[spark] abstract class Task[T](val stageId: Int, var partitionId: Int) extends Serializable{
  ...
  protected var _readTime: Long = 0
...
private[spark] object Task {
  ...
  def deserializeWithDependencies(serializedTask: ByteBuffer)
    ...
    for (i <- 0 until numFiles) {
      ...
      val fileinfo = dataIn.readLong()
      println(name+"-"+fileinfo)
      taskFiles(name) = fileinfo
    ...
    println("dependent numJars:"+numJars)
    for (i <- 0 until numJars) {
      val name = dataIn.readUTF()
      val fileinfo = dataIn.readLong()
      println(name+"-"+fileinfo)
      taskJars(name) = fileinfo
    }
