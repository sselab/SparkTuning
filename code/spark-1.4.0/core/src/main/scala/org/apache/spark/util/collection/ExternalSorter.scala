...
private[spark] class ExternalSorter[K, V, C](
  ...
  //trace
  private var shuffleRead:ShuffleRead=null
  private var shuffleWrite:ShuffleWrite=null
  private var dComputeOutRecs: Long = 0L
  private var computeTime:Long=0L
  private var cPartitionPhaseTime:Long=0L
  private var cCombPhaseTime:Long=0L
  private var dCombRecs:Long=0L
  private var cSortPhaseTime: Long = 0L
  private var dnumspills: Long = 0L
  private var dSpillRecs: Long = 0L
  private var cMergeReadTime: Long = 0L
  private var cMergeSortTime: Long = 0L
  private var cMergeCombTime: Long = 0L
  private var cMergeWriteTime: Long = 0L
  private var dSpillFileSize: Long = 0L
  private var dSpillBufferSize: Long = 0L
  private var dPerSpillRecs: Long =0L
  private var cSpillPhaseTime:Long =0L
  private var isMergeCombine:Boolean =false
  private var trueComputeTime:Long =0L
  ...
  def insertAll(records: Iterator[_ <: Product2[K, V]]): Unit = {
    ...
    //trace
    val shuffle=InternalProfiler.getTaskProfileForThreadId(Thread.currentThread().getId).setShuffleReadOrWrite(this.hashCode())
    if(shuffle.isInstanceOf[ShuffleRead]){
      shuffleRead=shuffle.asInstanceOf[ShuffleRead]
    }else{
      shuffleWrite=shuffle.asInstanceOf[ShuffleWrite]
    }
    ...
    if (shouldCombine) {
      ...
      while (records.hasNext) {
       ...
       // val startTime=System.nanoTime()
        ...
        val key=getPartition(kv._1)
        val startTime2=System.nanoTime()
        map.changeValue((key, kv._1), update)
        val startTime3=System.nanoTime()

        maybeSpillCollection(usingMap = true)
        //computeTime+=startTime1-startTime
        cPartitionPhaseTime+=startTime2-startTime1
        cCombPhaseTime+=startTime3-startTime2

      }
      //System.out.println("size------------------------"+GetObjectSize.getObjectSize(kv))
      trueComputeTime=System.nanoTime()-computeStartTime-cPartitionPhaseTime-cSpillPhaseTime-cSortPhaseTime-cCombPhaseTime
      computeTime=trueComputeTime
    //  objectSize=map.estimateSize()/elementsRead
      ...
      if (records.hasNext) {
        ...
        spillToPartitionFiles(
          WritablePartitionedIterator.fromIterator(records.map { kv =>
            ...
            val startTime2=System.nanoTime()
            cPartitionPhaseTime+=startTime2-startTime1
            ((key, kv._1), kv._2.asInstanceOf[C])
          })
        )
        computeTime=System.nanoTime()-computeStartTime-cPartitionPhaseTime-cSpillPhaseTime
      }
    } else {
      ...
      while (records.hasNext) {
       ...
       // val startTime = System.nanoTime()
        ...
        cPartitionPhaseTime += startTime2 - startTime1
        //computeTime += startTime1 - startTime
        buffer.insert(key, kv._1, kv._2.asInstanceOf[C])
        maybeSpillCollection(usingMap = false)
      }
      trueComputeTime=System.nanoTime()-computeStartTime-cPartitionPhaseTime-cSpillPhaseTime-cSortPhaseTime
      computeTime=trueComputeTime
     // objectSize=buffer.estimateSize()/elementsRead
    }

    //trace
    dComputeOutRecs+=elementsRead
    dnumspills=spillCount
    if(dnumspills!=0) {
      dSpillBufferSize = memoryBytesSpilled / dnumspills
      dSpillFileSize = diskBytesSpilled / dnumspills
      dPerSpillRecs =dSpillRecs /dnumspills
    }
    if(map.size==0){
      dSpillRecs+=elementsRead
      if(!bypassMergeSort) {
        if(dSpillRecs!=0) {
          objectSize = (buffer.estimateSize() + memoryBytesSpilled) / dSpillRecs
        }
      }
    }else{
      dSpillRecs+=map.size
      if(dSpillRecs!=0) {
        objectSize = (map.estimateSize() + memoryBytesSpilled) / dSpillRecs
      }
    }
    ...
    if(shuffleRead!=null){
        ...
        shuffleRead.byPassMergeSort=bypassMergeSort
        shuffleRead.spillEnabled=spillingEnabled
        shuffleRead.dnumspills=dnumspills
        shuffleRead.dSpillRecs=dSpillRecs
        shuffleRead.dPerSpillRecs=dPerSpillRecs
        shuffleRead.dSpillFileSize=dSpillFileSize
        shuffleRead.dSpillBufferSize=dSpillBufferSize
        shuffleRead.dCombRecs=dComputeOutRecs
        shuffleRead.cSpillPhaseTime=cSpillPhaseTime
        shuffleRead.objectSize=objectSize

    }else{
      shuffleWrite.byPassMergeSort=bypassMergeSort
      shuffleWrite.spillEnabled=spillingEnabled
      shuffleWrite.cCombPhaseTime=cCombPhaseTime
      shuffleWrite.cPartitionPhaseTime=cPartitionPhaseTime
      shuffleWrite.dnumspills=dnumspills
      shuffleWrite.dSpillFileSize=dSpillFileSize
      shuffleWrite.dSpillBufferSize=dSpillBufferSize
      shuffleWrite.dSpillRecs=dSpillRecs
      shuffleWrite.dPerSpillRecs=dPerSpillRecs
      shuffleWrite.cSpillPhaseTime =cSpillPhaseTime
      shuffleWrite.objectSize=objectSize
  ...
  private def spillToMergeableFile(collection: WritablePartitionedPairCollection[K, C]): Unit = {
   ...
   //trace
    ...
    try {
      ...
      cSortPhaseTime+=System.nanoTime()-startTime
      cSpillPhaseTime-=(System.nanoTime()-startTime)
      while (it.hasNext) {
    ...
    //trace
    dSpillRecs+=elementsPerPartition.sum
    cSpillPhaseTime+=System.nanoTime()-startTime1
  ...
  private def spillToPartitionFiles(iterator: WritablePartitionedIterator): Unit = {
    ...
    cSpillPhaseTime=iterator.spillPhaseTime
  }
...
//            if(shuffleWrite.cMergeCombTime!=0){
//              shuffleWrite.cMergeCombTime-=(shuffleWrite.cMergeSortTime+shuffleWrite.cMergeReadTime)
//            }
//            //shuffleWrite.cMergeSortTime -= shuffleWrite.cMergeReadTime
//          }else{
//            if(shuffleRead.cMergeCombTime!=0){
//              shuffleRead.cMergeCombTime-=(shuffleRead.cMergeSortTime+shuffleRead.cMergeReadTime)
//            }
//           // shuffleRead.cMergeSortTime -= shuffleRead.cMergeReadTime
//          }
          false
        }
      }
      ...
      override def next(): Product2[K, C] = {
        ...
        var interval=System.nanoTime()-startTime1
        val firstPair = firstBuf.next()

        if (firstBuf.hasNext) {
          val startTime3=System.nanoTime()
          heap.enqueue(firstBuf)
          interval+=(System.nanoTime()-startTime3)
        }
        cMergeSortTime=interval
        if(shuffleWrite!=null) {
          shuffleWrite.cMergeSortTime +=interval
          if(isMergeCombine) {
            shuffleWrite.cMergeCombTime -= interval
          }
        }else{
          shuffleRead.cMergeSortTime += interval
          if(isMergeCombine){
          shuffleRead.cMergeCombTime-=interval
          }
        }
        firstPair
      }
    ...
    if (!totalOrder) {
      ...
      new Iterator[Iterator[Product2[K, C]]] {
        ...
        override def next(): Iterator[Product2[K, C]] = {
          ...
          if(shuffleWrite!=null) {
            shuffleWrite.cMergeCombTime += System.nanoTime() - startTime
          }else{
            shuffleRead.cMergeCombTime += System.nanoTime() - startTime
          }
  ...
  private[this] class SpillReader(spill: SpilledFile) {
    ...
    def readNextPartition(): Iterator[Product2[K, C]] = new Iterator[Product2[K, C]] {
      ...
      override def hasNext: Boolean = {
        ...
        cMergeReadTime=System.nanoTime() - startTime
        if(shuffleWrite!=null) {
          shuffleWrite.cMergeReadTime += cMergeReadTime
          if(isMergeCombine) {
            shuffleWrite.cMergeCombTime -= cMergeReadTime
          }
        }else{
          shuffleRead.cMergeReadTime += cMergeReadTime
          if(isMergeCombine) {
            shuffleRead.cMergeCombTime -= cMergeReadTime
          }
        }
   ...
   def partitionedIterator: Iterator[(Int, Iterator[Product2[K, C]])] = {
    ...
    if (spills.isEmpty && partitionWriters == null) {
      ...
      if (!ordering.isDefined) {
        ...
        val startTime=System.nanoTime()
        val iter=groupByPartition(collection.partitionedDestructiveSortedIterator(None))
        cSortPhaseTime+=System.nanoTime()-startTime
        //trace
        if(shuffleRead!=null){
          shuffleRead.cSortPhaseTime=cSortPhaseTime
        }else{
          shuffleWrite.cSortPhaseTime=cSortPhaseTime
        }
        iter
      } else {
        // We do need to sort by both partition ID and key
        val startTime=System.nanoTime()
        val iter=groupByPartition(collection.partitionedDestructiveSortedIterator(Some(keyComparator)))
        cSortPhaseTime+=System.nanoTime()-startTime
        //trace
        if(shuffleRead!=null){
          shuffleRead.cSortPhaseTime=cSortPhaseTime
      ...
      val startTime=System.nanoTime()
      val iter=collection.partitionedDestructiveSortedIterator(comparator)
      cSortPhaseTime+=System.nanoTime()-startTime
      //trace
      if(shuffleRead!=null){
        shuffleRead.cSortPhaseTime=cSortPhaseTime
  ...
  def writePartitionedFile(
    ...
    if (bypassMergeSort && partitionWriters != null) {
      ...
      util.Utils.tryWithSafeFinally {
        ...
        //trace
        if(shuffleWrite!=null) shuffleWrite.cMergeWriteTime=System.nanoTime - writeStartTime
      }
     ...
     //trace
      ...
      if(shuffleWrite!=null) shuffleWrite.cMergeWriteTime=System.nanoTime - startTime1
    } else {
      ...
      for ((id, elements) <- this.partitionedIterator) {
        if (elements.hasNext) {
          ...
          for (elem <- elements) {
            ...
            cMergeWriteTime+=System.nanoTime()-startTime
          }
          writer.commitAndClose()
          val segment = writer.fileSegment()
      ...
      //trace
      if(shuffleWrite!=null) shuffleWrite.cMergeWriteTime=cMergeWriteTime
    }
