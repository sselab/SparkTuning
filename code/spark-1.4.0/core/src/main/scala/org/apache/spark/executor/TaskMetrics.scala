...
import org.apache.spark.TaskProfile
...
class TaskMetrics extends Serializable {
   ...
   * wenyanqi 2016-3-16
   */
  private var _taskprofile: TaskProfile = _
  def taskprofile: TaskProfile = _taskprofile
  private[spark] def setTaskProfile(value: TaskProfile) = _taskprofile = value
...
case class InputMetrics(readMethod: DataReadMethod.Value) {
  ...
  */
   ...
   * read Time spend
   */
  private var _readTime: Long = _
  def readTime: Long = _readTime
  def increadTime(readtime: Long): Unit = _readTime += readtime
}
