...
private[spark] class SortShuffleWriter[K, V, C](
  ...
  override def write(records: Iterator[Product2[K, V]]): Unit = {
    ...
    if (dep.mapSideCombine) {
      ...
      //trace
      InternalProfiler.getTaskProfileForThreadId(Thread.currentThread().getId).hashToReadOrWriteSorter.put(sorter.hashCode(),1)
      ...
      InternalProfiler.getTaskProfileForThreadId(Thread.currentThread().getId).hashToReadOrWriteSorter.put(sorter.hashCode(),1)
      InternalProfiler.getTaskProfileForThreadId(Thread.currentThread().getId).shuffleWrite.mapSideCombine=false
      sorter.insertAll(records)
    }
