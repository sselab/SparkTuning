...
class DAGScheduler(
  ...
  def taskEnded(
    ...
    if(taskInfo.gettingResultTime!=0&&taskInfo.finishTime-taskInfo.gettingResultTime>0) {
      taskMetrics.taskprofile.sendResultTime = (taskInfo.finishTime - taskInfo.gettingResultTime)*1000000
    }
    //taskMetrics.taskprofile.dResultBytes =taskMetrics.resultSize
    if(InternalProfiler.stageIdToTaskProfiles.get(task.stageId) == null){
      var taskprofiles = new util.ArrayList[TaskProfile]()
      taskprofiles.add(taskMetrics.taskprofile)
      InternalProfiler.stageIdToTaskProfiles.put(task.stageId,taskprofiles)
    }else{
      InternalProfiler.stageIdToTaskProfiles.get(task.stageId).add(taskMetrics.taskprofile)
    }
  ...
  private def newShuffleMapStage(
    ...
    DriverTracer.traceShuffleStageCreate(jobId,id,numTasks,"ShuffleMapStage",parentStages)
  ...
  private def newResultStage(
    ...
    DriverTracer.traceResultStageCreate(jobId,id,numTasks,"ResultStage",parentStages);
    stageIdToStage(id) = stage
    updateJobIdStageIdMaps(jobId, stage)
    stage
  }
  ...
  private def newOrUsedShuffleStage(
    ...
    if (mapOutputTracker.containsShuffle(shuffleDep.shuffleId)) {
      ...
      DriverTracer.traceUsedStage(stage.id)
    } else {
  ...
  private def markStageAsFinished(stage: Stage, errorMessage: Option[String] = None): Unit = {
    ...
    InternalProfiler.writeStageXML(stage.id)
